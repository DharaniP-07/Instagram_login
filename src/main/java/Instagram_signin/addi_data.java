...
import Instagram_signup.user_validations;
public class addi_data extends Scan {
	   static int c=3;
		...
		public static void pass(String mNumber) {
			...
			String old_password=Dao_signin.password(mNumber);
...
//			System.out.println(old_password);
		...
		int otp=Dao_signin.otp(mobileNumber);
		System.out.println(new_otp);
			System.out.println(otp);
		if(otp==new_otp)
			{
				Dao_signin.forgetPass(mobileNumber);
			} 
			else
			{
		...
		{
			String db_mobileNumber=Dao_signin.mobile(mobileNumber);
